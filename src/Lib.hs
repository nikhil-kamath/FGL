...
prop_CrossDFS (g :?=: g') ns =
  not (isEmpty g)
    && not (null $ intersect (nodes g) ns)
    ==> ds
    === ds'
  where
...
prop_CrossNearestNode' (NE g g' ns) =
  not (isEmpty g)
    ==> nns
    === nns'
  where
...
prop_CrossNearestDist' (NE g g' ns) =
  not (isEmpty g)
    ==> nns
    === nns'
  where
...
prop_CrossNearestPath' (NE g g' ns) =
  not (isEmpty g)
    ==> nns
    === nns'
  where
